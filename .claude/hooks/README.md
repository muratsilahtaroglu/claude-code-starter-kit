...
| `citation-gate.py` | `Stop`(--stop) | **Provenance:** do the artefacts our permanent records CITE exist in HEAD? `git commit -- <path>` skips an untracked file WITHOUT erroring, so a record can keep a reference the artefact never earned. Three classes, none silent: GHOST (on disk, not in HEAD) · UNRESOLVED (counted, never dropped) · BY DESIGN (.gitignore'd — a doc may name a file nobody commits). Resolves through `done/` so a `/keel-distill` sweep breaks nothing, normalises `..` before asking git, and self-tests its own pattern (exit 2 = tool broken, not "clean" — that is the MANUAL run; the registered `--stop` path reports and always exits 0, never blocking a session). Fires only when the .md set changed — an always-red gate teaches people to walk past it. Allowlist: `.claude/citation-allow`. rules §6.18 · §3.10 · §10.40 |
