...
| `pre-compact-snapshot.sh` | `PreCompact` | Writes a pre-compaction snapshot so state survives even an unclean compaction; when a compaction (usually AUTO) crosses a dirty tree with HANDOVER.md untouched it appends a durable `compact <trigger> STALE-DISK` line to `.claude/ritual-log` — the systemMessage dies with the context it warns, the line survives and the SessionStart hook reads it back |
