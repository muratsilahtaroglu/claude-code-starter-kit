...
| `LESSONS.md` | critical knowledge written **the moment it appears** (rules, must-run tests, gotchas, failures) — with your approval | cap in `.claude/keel-caps`; `/keel-distill` works BOTH exits: still true → promoted into rules/skill/ADR/docs, proven wrong → retired to `docs/lessons-retired.md` with one corrective line left behind |
