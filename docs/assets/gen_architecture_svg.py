...
GROUPS = [
    ...
    (".claude/", "#1c7ed6", [
        ...
        ("skills/", "17 /keel-* workflows — continue · team · agent-team · handover"),
        ("agents/", "subagents — researcher · verifier · auditor"),
        ("rules/", "optional path-scoped rules"),
        ("team-addresses.py · claude-launch-wrapper.sh", "agent-team addressing — twin/attachment check · a resume keeps its name"),
        ("ritual-report.py", "renders the telemetry /keel-stats reads"),
...
FIXES = [
    ...
    (("Parallel sessions collide",
      "Several chats in one repo have no merge layer: two writers clobber shared memory silently, a reopened window loses the address peers reach it by, and two workers settle things no file records."),
     ("A star-topology crew",
      "One writer per shared file (the orchestrator); worker→worker messaging blocked by a hook; identity kept on disk and the messaging address restored automatically on every resume by the launch wrapper.")),
