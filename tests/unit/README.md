...
the template's status alternation · the audit clock's marker parser: prose after the sha never
contributes hex, an unreadable marker says so instead of counting all history, a missing one does not). Origin: three security bypasses shipped
