# 2026-09-06 — alice_v2 → kit promotion review (v0.8.36)

**Source:** `alice_v2/reports/team/muratsilahtaroglu/observer/keel_agent_brief_2026-09-06.md` (25 candidates,
written by @observer against alice's copy of kit v0.8.34). **Method (rules §10.37):** every candidate was
measured on BOTH sides before a verdict — alice's commit/file/test opened, the kit's current file diffed —
not taken from the brief's sentence. Alice was not touched (kit-first; alice pulls via `/keel-update`).

Verdict vocabulary: **ENTERS** · **NARROWER form enters** · **ALREADY in kit** · **DOES NOT ENTER**.

| # | Candidate | alice evidence (measured) | Verdict | Reason (one line) | Landed in kit |
|---|---|---|---|---|---|
| A1 | Twin resolver: live = NEWEST pid per session-id, not `attached` | `c1366ff` · `.claude/team-addresses.py` · 21-cell test matrix passes | **ENTERS** | Kit's 09-03 axis (`attached`) measured blind: per VS Code BUILD, 11/11 read attached; alice's version is a strict superset of the kit's own file | `.claude/team-addresses.py` · `tests/unit/test_keel_team_addresses.py` (21 cells) · `docs/steering.md` twin paragraph rewritten |
...
| A4 | Commit-time fast test subset | idea approved (§7d İ5) — **no target in alice's Makefile, not measured** | **NARROWER** (opt-in) | Unproven mechanism; a mandatory commit-time suite would block docs-only commits. Shipped as a `test-fast` target + a commented pre-commit hook | `Makefile` `test-fast` · `.pre-commit-config.yaml` (commented block) |
...
| A6 | compact-gate checks "approved suggestion has a governance diff" | idea only, no hook in alice | **DOES NOT ENTER** | "Was an approval landed" is not machine-decidable at PreCompact; covered as a RECORD by E1's follow-up table instead | — (see E1) |
...
| B3 | Owner-absence protocol | `rules.md §10.42 İ16`; ran live 09-05/06: 4 lanes, 0 owner questions, Review 17→6 | **ENTERS** | Measured to work; kit had nothing for "owner away" | rules §10.42 "Owner absence" |
| B4 | LESSONS cold layer (`docs/retro/`) | `rules §9.33 İ6` · `docs/retro/README.md`; LESSONS stuck 1000/1000, 82/132 single-copy | **ENTERS** | Kit had retire (wrong) and promote (3+ uses) — no exit for true-but-uncited; the proxy criterion is labelled as a proxy | `docs/retro/README.md` · `keel-distill` §2 COOL · rules §9.33 · LESSONS header · `docs/memory-files.md` (E) |
...
| B6 | `/keel-stats` process metrics | audit §0 table, derived by hand; **no script in alice** | **ENTERS** (built here first) | Numbers nobody sees until someone counts; generator is deterministic git+tree, classifier written down | `.claude/ritual-report.py` `process_metrics()` · `keel-stats` SKILL |
| C1 | Tiers T0/T1/T2 decide `## Review` | ADR-0012 + `§10.41 İ2`; Review 17→15, Now 11→7 on first pass; 45 items/37 no owner part before | **ENTERS** | Kit routed everything with "owner may waive trivia" — ceremony was habit, not risk | rules §10.41 TIER · `keel-continue` ROUTE · orchestrator charter text |
...
| C6 | Index GENERATED, `done/` retired | `rules §10.40 İ8` — **generator not written in alice** | **DOES NOT ENTER (deferred)** | Unmeasured mechanism that changes who writes status; §10.37 ground-before-build. Revisit when alice's generator exists and has run | — |
...
| D3 | `verifier` requirement vs sub-agent policy | `rules §10.41 İ10` | **ENTERS** (merged into C1) | Kit §4.11 said "an INDEPENDENT sub-agent" — unreachable where workers are denied sub-agents; routed review named as the alternative, sub-agent stays mandatory for the orchestrator's own changes | rules §10.41 TIER paragraph |
...
counting discipline · alice's teardown cases — all alice-specific. The teardown PROCEDURE itself was already
...
## What alice should do next (its own decision, via `/keel-update`)
Pull v0.8.36. Expected REVIEW hunks: `rules.md` (alice's §10.40–42 already carry the Turkish originals of
these paragraphs — keep alice's wording, take the kit's structure), `Makefile` (`team-clean` comment text
differs), `.pre-commit-config.yaml`. TOOLING lands whole: the two ported hooks are byte-identical to
alice's except for corrected header docstrings; `observer.md`'s kit core is what alice already has above
its marker line.
